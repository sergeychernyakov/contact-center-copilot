...
Two checks:
1. Numeric accuracy (deterministic regex): every number in the narrative MUST
   appear in the source MetricsSet or benchmark chunks.
2. LLM-based faithfulness rating against the same context.
"""
...
is at least {threshold_relaxed}, set passes=True and add a review_reason.
"""
)


def _allowed_numbers(state: CopilotState) -> set[str]:
    """Build a set of every number that's allowed to appear in the narrative."""
    allowed: set[str] = set()
    metrics = state.get("metrics")
    if metrics:
        for m in metrics.metrics:
            allowed.add(f"{m.value:g}")
            # Allow rounded versions
...
async def critique_report(state: CopilotState) -> dict:
    ...
    # Combine: take LLM's faithfulness + deterministic numeric
    final = CriticReport(
